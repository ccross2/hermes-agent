...
def get_stt_model_from_config() -> Optional[str]:
    """Return the configured STT model name.

    Preference: user-supplied ``stt.model`` wins; otherwise fall back to
    the per-provider override ``stt.{provider}.model``. This ordering
    keeps explicit user config authoritative and avoids DEFAULT_CONFIG
    per-provider defaults (e.g. ``stt.local.model = "base"``) silently
    shadowing an explicit ``stt.model`` set by the user.

    Uses ``_load_stt_config()`` so every STT codepath reads from the
    same config layer (and tests can mock load_config). Returns
    ``None`` when no STT config is present.
    """
    stt = _load_stt_config() or {}
    if not stt:
        return None
    explicit = stt.get("model")
    if explicit:
        return explicit
    provider = stt.get("provider")
    per_provider = (stt.get(provider) or {}) if provider else {}
    return per_provider.get("model")
